    ...
    def list_flights(self, criteria: bytes = b""):
        ...
        try:
            ...
            for flight_info in flights:
                ...
                # Fix: decode bytes to str for each path segment before joining (robust)
                if flight_info.descriptor.path:
                    path_segments = [seg.decode('utf-8', errors='ignore') if isinstance(seg, bytes) else str(seg) for seg in flight_info.descriptor.path]
                ...
                for i, endpoint in enumerate(flight_info.endpoints):
                    ...
                    ticket_str = ticket_bytes.decode('utf-8', errors='ignore') if isinstance(ticket_bytes, bytes) else str(ticket_bytes)
                    logger.info(f"      Endpoint {i+1}:")
                    logger.info(f"        Ticket: {ticket_str}")
                    # Fix: ensure all location URIs are str
                    logger.info(f"        Locations: {', '.join(str(loc.uri) for loc in endpoint.locations) if endpoint.locations else 'None'}")
                logger.info("-" * 30)

...
class SimpleFlightClient:
    def __init__(self, host: str = "127.00.1", port: int = 50051):
    ...
    def generate_batch_table(self, batch_index: int, num_rows: int) -> pa.Table:
        # Create fields with names, types, and the crucial Iceberg field ID metadata
        fields = [
        ...
        # Create the schema from these fields
        ...
        return pa.Table.from_arrays(
            ...
            schema=schema # Use the schema with metadata here
        )
...
async def async_send_batches(client: SimpleFlightClient, batch_descriptor: flight.FlightDescriptor, schema: pa.Schema, rows_per_batch: int, num_batches: int) -> bool:
    ...
    try:
        def blocking_send_operation():
            ...
            try:
                ...
                logger.error(f"Flight error during async do_put for '{batch_descriptor.path.decode()}': {e}")
                success = False
            except Exception as e:
                logger.error(f"Unexpected error during async do_put for '{batch_descriptor.path.decode()}': {e}")
                success = False
            finally:
                try:
        ...
        logger.error(f"Error initiating blocking send operation for '{batch_descriptor.path.decode()}': {e}")
        success = False
    ...
    if success:
        ...
        logging.error(f"Failed to send all batches for '{batch_descriptor.path.decode()}'.")
...
async def main():
    ...
    # The schema used for do_put must come from a table that has the metadata
    # The generate_batch_table function now ensures this.
    ...
    if not success:
        logger.error(f"Asynchronous batch sending failed for path: {batch_descriptor.path.decode()}")
    else:
        logger.info(f"Asynchronous batch sending completed successfully for path: {batch_descriptor.path.decode()}")
